...
function ProfilePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    //Ici, j’ai complètement supprimé  birthDate.
    try {
      const payload = {
        firstName,
        isActive: true,
      ...
      <div className="bg-white rounded-xl shadow-xl p-6 sm:p-8 w-full max-w-md mx-2">
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-4">
          <div>
            ...
            <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="w-full border px-3 py-2 rounded-lg focus:ring-2 focus:ring-purple-500 focus:outline-none" />
          </div>
        ...
        {showModal && (
          <div className="fixed inset-0  bg-opacity-40 flex items-center justify-center z-50 p-2">
            <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-sm text-center">
              ...
              <button className="mt-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition duration-300" onClick={() => setShowModal(false)}>
                Fermer
              </button>
            </div>
          </div>
